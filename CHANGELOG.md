...
### Changed
- **The demo catalog is now synthetic and MIT, generated by this repo.** The 720
  products used to be a subset of a third-party Amazon dataset whose upstream grants no
  redistribution license. `backend/scripts/generate_catalog.py` now builds the catalog
  from committed vocabulary (`src/edgereco/catalog/synthetic_vocab.json`: invented
  brands, titles, feature text, prices and ratings on the same 12 categories). Same
  seed, same bytes. The signed bundle (sequence 12, same pinned key), embeddings,
  co-occurrence sessions, parity fixtures and product images (generated SVG cards; the
  720 downloaded product photos are gone) were all rebuilt from it with
  `rebuild_example_bundle.py --from-source`. `curate_demo_catalog.py` is removed.
- Footer, NOTICE, README and docs now carry one attribution statement, read from
  `frontend/app/src/catalog-attribution.json` and pinned by `catalogAttribution.test.tsx`.
- Relevance floors re-baselined for the new catalog (the ranker is unchanged); the
  before/after numbers are recorded in `test_relevance_baseline.py`.
