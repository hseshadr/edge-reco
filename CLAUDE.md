...
- `backend/` — Python project root (pyproject.toml + uv.lock + .venv live here)
  ...
  - `backend/examples/catalog/` — committed signed 720-product **synthetic** catalog bundle (12 categories × 60, invented brands, MIT); `examples/source/catalog.csv` is generated by `scripts/generate_catalog.py` from `src/edgereco/catalog/synthetic_vocab.json`, then `scripts/rebuild_example_bundle.py --from-source` runs `build-catalog` → `index` → sign
