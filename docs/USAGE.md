...
An ordinary search box matches words. Search a real 720-product catalog for
"something for my aching back" by words alone and the top result is a parking sign,
because "back" appears in its title. EdgeReco runs the word match and a meaning match,
then merges them, and the massage gun comes first.
...
Real output (fresh clone, 25 Sep 2026):
...
| --- | --- | --- |
| Demo data | `backend/examples/catalog/` | A committed, signed bundle of 720 real Amazon products, 60 in each of 12 categories, so re-ranking visibly personalizes. Nimbus and the offline demo use this. |
...
The 720 products are a balanced subset of the **Amazon Reviews 2023** dataset (item
metadata) from the McAuley Lab at UC San Diego
([amazon-reviews-2023.github.io](https://amazon-reviews-2023.github.io/), released for
research use; cite Hou et al., *arXiv:2403.03952*). It is produced by
`scripts/curate_demo_catalog.py`, then `edgereco build-catalog`, `edgereco index` and
`edgereco bundle`, and you can regenerate it with the same commands.

That attribution is not a license to the content: the listings, titles and images come
from Amazon.com and remain subject to Amazon's terms. See [NOTICE](../NOTICE), and check
your rights before redistributing.
