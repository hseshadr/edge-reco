...
#: RE-BASELINED 2026-09-26 because the CATALOG changed, not the ranker. The demo
#: catalog moved from a third-party Amazon dataset (no license to redistribute) to the
#: synthetic catalog generated by scripts/generate_catalog.py. The ranker, the golden
#: queries and every label node are unchanged; the products under those nodes are
#: new. The golden queries were worded against the OLD descriptions (three natural
#: anchors — "iphone", "apple", "android" — name real products the synthetic catalog
#: never mentions), so the natural segment is expected to lose some recall. Measured
#: on the same ranker, old catalog -> synthetic catalog:
#:     natural       nDCG@10 0.5390 -> 0.5178  R@10 0.6742 -> 0.5121  P@10 0.3033 -> 0.3366
#:                   nDCG@24 0.6330 -> 0.6160  R@24 0.8997 -> 0.7358
#:     taxonomy-word nDCG@10 0.8943 -> 0.9631  R@10 0.1481 -> 0.1583  P@10 0.9000 -> 0.9500
#:                   nDCG@24 0.8936 -> 0.9368  R@24 0.3539 -> 0.3680
#:     negative results 0 -> 2 ("treadmill" returns a yoga mat and an exercise mat)
#: Lowering a floor is only legitimate when the thing measured changed; that is the
#: case here, and it is argued in the PR that does it. From this baseline on, the
#: ratchet works as before: ranking changes may raise these numbers, never lower them.
#:
#: The previous baseline, measured 2026-08-03 on the unmodified ranker (old catalog).
#:
...
#: wrong by construction, so the honest number is zero. Was 192 (8 queries x the full
#: 24) before the absolute floor, then 0 on the old catalog. It is 2 on the synthetic
#: catalog: "treadmill" returns a yoga mat and an exercise mat, which clear the
#: absolute floor as fitness neighbours. Pinned at 2 so it cannot grow; the fix belongs
#: in the ranker.
MAX_NEGATIVE_RESULTS: Final[int] = 2
...
def test_export_is_the_committed_demo_baseline(export: RelevanceExport) -> None:
...
def test_the_negative_guard_refuses_a_floorless_ranking(export: RelevanceExport) -> None:
    ...
    """
    full_page = next(
        (q.ranked_ids for q in export.in_segment("natural") if len(q.ranked_ids) == export.k),
        (),
    )
