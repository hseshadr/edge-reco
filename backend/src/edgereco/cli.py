...
def retrain(
    ...
    sessions_path: Annotated[
        Path | None,
        typer.Option("--sessions", help="JSONL session log to recompute co-occurrence from"),
    ] = None,
    ...
    """Recompute popularity + co-occurrence from collected events and republish.

    The cloud half of the flywheel: sync the current bundle, fold in engagement
    (popularity) pulled from the collector and a session log (co-occurrence), and
    republish a freshly signed bundle (new ``latest``) — reusing the prebuilt FAISS
    index verbatim. Both are pure DATA transforms; the scoring formula never changes.
    """
...
def _load_session_logs(sessions_path: Path | None) -> list[SessionLog]:
    """Read a JSONL session log file into typed ``SessionLog`` lines (empty if None).

    A malformed or schema-violating line raises a ``ValueError`` naming the file and
    the 1-based line number, so a maintainer can locate the bad line directly instead
    of getting a bare JSON/validation error with no position.
    """
    if sessions_path is None:
        return []
    from edgereco.reco.cooccurrence import SessionLog

    logs: list[SessionLog] = []
    for lineno, line in enumerate(sessions_path.read_text(encoding="utf-8").splitlines(), start=1):
        if not line.strip():
            continue
        try:
            logs.append(SessionLog.model_validate_json(line))
        except ValueError as exc:
            msg = f"{sessions_path}: line {lineno}: malformed session log: {exc}"
            raise ValueError(msg) from exc
    return logs


def _load_sessions(sessions_path: Path | None) -> list[Session]:
    """Read a JSONL session log into the ``(product_id, event_type)`` session shape."""
    from edgereco.reco.cooccurrence import sessions_from_logs

    return sessions_from_logs(_load_session_logs(sessions_path))
...
# ---------------------------------------------------------------------------
# audit (read-only: explain what a retrain would change, and why)
# ---------------------------------------------------------------------------
...
def audit(
    ...
    sessions_path: Annotated[
        Path | None, typer.Option("--sessions", help="JSONL session log to replay")
    ] = None,
    alpha: Annotated[float, typer.Option(help="Engagement boost weight in [0, 1]")] = 0.5,
    cache_dir: Annotated[Path, typer.Option(help="Scratch dir for the read-only sync")] = Path(
        ".audit-cache"
    ),
) -> None:
    """Report what a retrain would change — never publishes, never touches inference.

    Syncs the current signed bundle, replays the collected session log to preview
    popularity + co-occurrence, and prints the event counts, top popularity movers,
    and changed co-occurrence edges behind the next ranking update.
    """
    report = _build_audit(bundle_base_url, verify_key_path, sessions_path, alpha, cache_dir)
    from edgereco.reco.audit import render_audit_table

    typer.echo(render_audit_table(report))
...
def _build_audit(
    ...
    verifier = Ed25519Verifier.from_public_bytes(verify_key_path.read_bytes())
    local = sync_and_materialize(base_url=bundle_base_url, cache_root=cache_dir, verifier=verifier)
    meta = CatalogMeta.model_validate_json((local / "catalog_meta.json").read_bytes())
    logs = _load_session_logs(sessions_path)
    inputs = audit_inputs(
    ...
    return build_audit_report(
        ...
        after=inputs.after,
        before_cooccurrence=inputs.before_cooccurrence,
