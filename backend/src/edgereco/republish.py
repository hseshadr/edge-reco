...
def retrain_and_republish(
    *,
    ...
    """Sync the current bundle, fold in engagement, and republish a signed bundle.

    The prebuilt FAISS ``vector/`` is reused verbatim (embeddings depend on text,
    not popularity), so no re-encode is needed. Fails closed on a bad signature.
    """
    ...
    staging = _stage_catalog(materialized, blended, cache_root / "staging")
    _publish(
        staging=staging,
...
def _stage_catalog(materialized: Path, products: list[Product], staging: Path) -> Path:
    """Write the recomputed catalog and reuse the prebuilt ``vector/`` verbatim."""
