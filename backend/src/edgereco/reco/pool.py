...
def _affinity_score(product: Product, profile: SessionProfile, weights: ScoringWeights) -> float:
...
def _affinity_matches(
    catalog: list[Product], profile: SessionProfile, weights: ScoringWeights
) -> list[Product]:
    """Every product the session has shown affinity for (rerank orders them later)."""
    return [p for p in catalog if _affinity_score(p, profile, weights) > 0.0]
...
def select_candidate_pool(
    ...
    if fewer than `limit`), else popularity top-N. The ``affinity_first`` policy."""
    size = min(limit * 5, len(catalog))
    if not _has_affinity(profile):
        pool = _popularity_top(catalog, size)
    else:
        matches = _affinity_matches(catalog, profile, weights)
...
def popularity_pool(catalog: list[Product], limit: int) -> list[SearchResult]:
    """The ``popularity`` policy: the most-popular ``limit*5`` products."""
    size = min(limit * 5, len(catalog))
    pool = _popularity_top(catalog, size)
    return [SearchResult(product=p, score=p.popularity_score) for p in pool]


def freshness_pool(catalog: list[Product], limit: int) -> list[SearchResult]:
    """The ``freshness`` policy: the freshest ``limit*5`` products."""
    size = min(limit * 5, len(catalog))
    pool = sorted(catalog, key=lambda p: p.freshness_score, reverse=True)[:size]
    return [SearchResult(product=p, score=p.freshness_score) for p in pool]
