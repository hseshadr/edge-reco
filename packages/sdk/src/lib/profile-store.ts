...
export async function createProfileStore(storage?: ProfileStorage): Promise<ProfileStore> {
  ...
  return {
    ...
    async applyClick(item: CatalogItem): Promise<void> {
      ...
      for (const tag of item.tags) {
        state.tagAffinity[tag] = bumpCapped(state.tagAffinity[tag], SCORING_CONSTANTS.clickTagBump);
      }
